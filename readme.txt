...
This plugin comes with 5 optional widgets.
...
2. If desired, place the widget files in the wp-content/mu-plugins or the plugins folder. It's recommended that you use the plugins folder and only enable these widgets on blogs that need them. They may be confusing for some users.
...
* fixed a bug with the blog admin's select drop down not showing currently selected blog.
